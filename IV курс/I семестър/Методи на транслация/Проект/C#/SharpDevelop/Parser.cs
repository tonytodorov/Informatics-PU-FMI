...
namespace scsc
{
    public class Parser
    {
        ...
        public void ifStatement(Type type)
        {
        ...
        public void whileStatement(Type type)
        {
            // Emit
        ...
        public void stopStatement(Type type)
        {
        ...
        public bool IsStatement()
        {
            ...
            else if (CheckKeyword("break") || CheckKeyword("continue") || CheckKeyword("return"))
            {
                stopStatement(type);
            }
        ...
        {
            // Define the pattern
            string constantPattern = @"^([+-])(\d+|\w+)|""([^""]*)""$";

            // Check match
            return Regex.IsMatch(input, constantPattern);
        }

        // [16] Variable = ['+'|'-'] VarIdent
        public void ParseVariableAssignment(string input)
        {
            // Define the pattern
            ...
            if (match.Success)
            {
                // Access the sign and variable identifier
                string sign = match.Groups[1].Value;
                string varIdent = match.Groups[2].Value;
            }
            else
            {
                Console.WriteLine("Syntax error: Invalid variable assignment.");
            }
        }

        public bool isSkipStatement()
        {
